...
const STOP_WORDS = new Set([
  ...
  "successful", "seeking", "skilled", "updated", "veteran", "workforce", "workplace"
]);
...
function optimizeResume(resumeText, jdText, instructionText = "") {
  ...
  return {
    ...
    warnings: unsupported.map((keyword) => `${sentenceCase(keyword)} was requested by the job description but was not added because it is not clearly supported by the default.`),
    suggestions: {
...
function buildCompleteResumeDraft(resumeText, suggestions, matched, missing, targetKeywordCount = matched.length + missing.length) {
  ...
  if (!targetSkills.length) return [...contactLines, "", ...bodyLines].join("\n");
...
function buildAiPrompt(resumeText, jdText, instructionText, matched, missing) {
  ...
  return [
    ...
    "```",
    ...(extraInstructions
      ? ["", "Additional optimization instructions:", "```", extraInstructions, "```"]
      : []),
    "",
    "Final output:",
    "Please create and provide the complete optimized resume in the same structure."
  ].join("\n");
}
...
async function extractResumeText(file) {
  ...
  const extension = (name.lastIndexOf(".") > -1 ? name.slice(name.lastIndexOf(".") + 1).toLowerCase() : "");
  ...
  if (extension === "pdf") {
    ...
    try {
      ...
      for (let index = 1; index <= pdf.numPages; index += 1) {
        ...
        pages.push(content.items.map((item) => (item.str != null ? item.str : "")).join(" "));
        await page.cleanup();
      }
      await pdf.destroy();
      return cleanExtractedText(pages.join("\n"));
    } catch (err) {
      throw new Error(`Could not read this PDF. ${err && err.message ? err.message : "The file may be scanned or password protected."}`);
    }
  }
...
function buildPdfBuffer(resumeText) {
  ...
  if (headerLines.length) {
    drawCentered(headerLines[0], { bold: true, fontSize: 20, lineHeight: 21, color: "0.07 0.30 0.56 rg" });
  ...
  for (const item of bodyItems) {
    ...
    if (item.type === "section") {
      section = line.toUpperCase();
      addVerticalSpace(6);
      drawText(section, marginX, { bold: true, fontSize: 11, lineHeight: 13, color: "0.07 0.18 0.56 rg" });
      drawRule(y + 8.2);
      addVerticalSpace(2);
      continue;
    }
    ...
    if (item.type === "bullet") {
      const wrapped = wrapPdfText(line, contentWidth - 32, 9.15);
    ...
    const isSkillsLine = section.includes("SKILLS");
    const fontSize = isSkillsLine ? 9.1 : 9.35;
  ...
  for (const pageOps of pageStreams) {
    ...
    const contentId = addPdfObject(objects, `<< /Length ${utf8ByteLength(stream)} >>\nstream\n${stream}\nendstream`);
  ...
  const xrefOffset = utf8ByteLength(pdf);
  pdf += `xref\n0 ${objects.length + 1}\n`;
...
async function analyze(input) {
  ...
  const pastedResume = String(input?.resumeText || "").trim();
  const prompt = String(input?.prompt || "").trim();

  if (!jd || (!file && !pastedResume)) {
    throw new Error("Please paste a job description and provide a resume file or pasted resume text.");
  }

  const resumeText = pastedResume || await extractResumeText(file);

  if (!resumeText) {
    throw new Error("I could not read the default resume.");
  }

  const result = optimizeResume(resumeText, jd, prompt);

  return {
    fileName: file?.name || "default-resume.txt",
    resumePreview: resumeText.slice(0, 1200),
    ...result
  };
}

export { analyze, analyzeResume, buildDocxBuffer, buildPdfBuffer };